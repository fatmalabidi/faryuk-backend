...
func GetComments(w http.ResponseWriter, r *http.Request) {
	...
	if username == adminUsername {
		// create the channel to receive the comments and error
		ch := make(chan types.CommentWithErrorType)

		dbHandler.GetCommentsByResult(idResult, ch)
		result := <-ch
		comments = []types.Comment{result.Comment}
		err = result.Err
		...
		if result == nil {
			...
			ch := make(chan types.CommentsWithErrorType)
